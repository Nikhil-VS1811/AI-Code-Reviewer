...
class ReviewPdfBuilder:
    ...
    def build(self) -> bytes:
        self._draw_header()
        self._add_section_title("Review Overview")
        self._add_key_value("Review ID", self.review.id)
        self._add_key_value("Submission ID", self.review.submission_id)
        self._add_key_value("Overall Score", f"{self.review.overall_score}/100")
        self._add_key_value("Security Score", f"{self.review.security_score}/100")
        self._add_key_value("Performance Score", f"{self.review.performance_score}/100")
        self._add_key_value("Maintainability Score", f"{self.review.maintainability_score}/100")
        self._add_key_value("Readability Score", f"{self.review.readability_score}/100")

        self._space(12)
        self._add_section_title("Summary")
        self._add_paragraph(self.review.summary)

        self._space(14)
        self._add_section_title("Review Comments")
        if self.review.comments:
            for index, comment in enumerate(self.review.comments, start=1):
                self._add_comment_block(index, comment)
        else:
            self._add_paragraph("No review comments were generated.")
    ...
    def _draw_header(self) -> None:
        ...
        self._add_text("AI Code Review Report", x=MARGIN, y=PAGE_HEIGHT - 54, size=22, font="F2", color="1 1 1")
        self._add_text(
            f"Review #{self.review.id} | Submission #{self.review.submission_id}",
            x=MARGIN,
            y=PAGE_HEIGHT - 78,
            size=11,
    ...
    def _ensure_space(self, height: int) -> None:
        ...
        self.page = self._new_page()
        self.y = PAGE_HEIGHT - MARGIN
        self._add_text("AI Code Review Report", size=12, font="F2", color="0.07 0.10 0.18")
        self._space(18)
...
def generate_review_pdf(review: Review) -> bytes:
    return ReviewPdfBuilder(review).build()
